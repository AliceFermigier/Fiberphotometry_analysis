...
for exp in os.listdir(analysis_path):
    for session in os.listdir(exp):
        mouse_path = analysis_path / 'CD1/'
        
        #get the name of the mouse, session, etc
        # '/' on mac, '\\' on windows
        ...
        fiberpho_path = str(mouse_path) + '/' + mouse + 'dFFfilt.csv'
        camera_path = str(mouse_path) + '/' + mouse + 'camera.csv'
...
list_BOI = ['Exploration fam', 'Exploration new'] 
...
def behav_process(fiberbehav_df, list_BOI):
    ...
    behavprocess_df.to_excel('/Users/alice/Desktop/fbprocess.xlsx')

    
    return(fiberbehav_df, behavprocess_df)
    
#%%
def PETH_onset(behavprocess_df, BOI):
    """
    ...
    --> Parameters
        ...
        BOI = behaviour of interest (must have the same name as behavprocess column name)
    --> Returns
    ...
    #creates array of fiberpho data centered on onset
    list_ind_onset = np.where(behavprocess_df[BOI] == 1)[0].tolist()
    if list_ind_onset[-1]+POST_TIME*SAMPLERATE >= len(behavprocess_df):
        list_ind_onset.pop(-1)
    list_i = np.arange(len(list_ind_onset), dtype = int).tolist()
    PETHo_array = np.zeros((len(list_ind_onset),(POST_TIME+PRE_TIME)*SAMPLERATE+1))
    print(PETHo_array, len(PETHo_array))
    for (ind_onset, i) in zip(list_ind_onset, list_i) :
        #calculates baseline F0 on time window before onset of behaviour (from PRE_TIME to PRE_ONSET)
        F0 = np.mean(behavprocess_df.loc[ind_onset-PRE_TIME*SAMPLERATE:ind_onset-PRE_ONSET*SAMPLERATE, 'Denoised dFF'])
        print(F0)
        #creates array with dFF normalized on F0
        print(behavprocess_df.loc[ind_onset-PRE_TIME*SAMPLERATE:(ind_onset+POST_TIME*SAMPLERATE), 'Denoised dFF'],
              len(behavprocess_df.loc[ind_onset-PRE_TIME*SAMPLERATE:ind_onset+POST_TIME*SAMPLERATE, 'Denoised dFF']))
        PETHo_array[i] = (behavprocess_df.loc[ind_onset-PRE_TIME*SAMPLERATE:ind_onset+(POST_TIME*SAMPLERATE), 'Denoised dFF']-F0)
    
    return(PETHo_array)

def PETH_withdrawal(behavprocess_df, BOI):
    """
    Creates dataframe of fiberpho data centered on bout withdrawal for BOI
    --> Parameters
        behavprocess_df : pd dataframe, aligned fiberpho and behav data for 1 mouse
        BOI : behaviour of interest
    --> Returns
        PETHw_array : np array, normalized fiberpho data centered on withdrawal
    """
    
    #set time window relative to exploration withdrawal
    PRE_TIME = 3
    POST_TIME = 7
    PRE_WITHD = 0.5
    ...
    #creates array of fiberpho data centered on withdrawal
    list_ind_withd = np.where(behavprocess_df[BOI] == -1)[0].tolist()
    if list_ind_withd[-1]+POST_TIME*SAMPLERATE >= len(behavprocess_df):
        list_ind_withd.pop(-1)
    list_i = np.arange(len(list_ind_withd), dtype = int).tolist()
    PETHw_array = np.zeros((len(list_ind_withd),(POST_TIME+PRE_TIME)*SAMPLERATE+1))
    for (ind_withd, i) in zip(list_ind_withd, list_i) :
        #calculates baseline F0 on time window before withdrawal of behaviour (from PRE_TIME to PRE_WITHD)
        F0 = np.mean(behavprocess_df.loc[ind_withd-PRE_TIME*SAMPLERATE:ind_withd-PRE_WITHD*SAMPLERATE, 'Denoised dFF'])
        #creates array with dFF normalized on F0
        PETHw_array[i] = (behavprocess_df.loc[ind_withd-PRE_TIME*SAMPLERATE:ind_withd+POST_TIME*SAMPLERATE, 'Denoised dFF']-F0)
    
    return(PETHw_array)

def plot_PETH(PETH_data, BOI):
    """
...
def PETH_allsubjects(subjects_df):
    ...
    for mouse in os.listdir(analysis_path):
        behavprocess_df = pd.read_csv(analysis_path / mouse / 'fbprocess.csv')
...
#%% Tests PETH
fiberbehav_df.to_excel('C:/Users/Alice/Desktop/fiberbehav.xlsx')
fiberbehav2_df.to_excel('C:/Users/Alice/Desktop/fiberbehav2.xlsx')
fbprocess_df.to_excel('C:/Users/Alice/Desktop/fbprocess.xlsx')
print(fiberbehav2_df, fbprocess_df)
...
#%% Test plotting PETH
PETH_data = PETH_withdrawal(fbprocess_df, 'Exploration new')
plot_PETH(PETH_data, 'Exploration new')