...
def PETH(behavprocess_df, BOI, event, timewindow, EVENT_TIME_THRESHOLD, PRE_EVENT_TIME):
    """
    ...
    #take the beginning of the behaviour as the beginning of the slope : min(dFF) on 2 secs before entry
    if event == 'onset':
        for i,ind_onset in enumerate(list_ind_event):
            list_ind_event[i] = behavprocess_df.loc[ind_onset-2*sr:ind_onset, 'Denoised dFF'].idxmin()
